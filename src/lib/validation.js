let validation = {
	...
	email (value) {
		if(value.length < 5) throw 'The email length must be more than 4'
		if( !(/[a-z]/).test(value) ) throw 'The email must include letters!'
		if( !(/[@]/).test(value)) throw 'The email must include @ !'
        if( !(/[.]/).test(value)) throw 'The email must include . '
		return value
	},
